	...
	jQuery(document).ready(function(){
		if(jQuery().jPlayer) {
			jQuery("#jquery-jplayer-<?php the_ID(); ?>").jPlayer({
				...
				cssSelectorAncestor: "#jp-interface-<?php esc_js( the_ID() ); ?>",
				supplied: "<?php echo esc_js( implode(',',$formats) ); ?>"
			});
		}
	});
	</script>
	...
	<div class="post-format">		
		<div class="image-container">
			<?php if ( has_post_thumbnail() ) {	
				...
				if ( isset($caption) && $caption ) echo '<div class="image-caption">'.esc_html( $caption ).'</div>';
			} ?>
		</div>
...
<?php if ( has_post_format( 'gallery' ) ): // Gallery ?>
	...
	<div class="post-format">
		...
		<script type="text/javascript">
			...
			jQuery(document).ready(function(){
			 var firstImage = jQuery('#flexslider-<?php echo esc_js( the_ID() ); ?>').find('img').filter(':first'),
				checkforloaded = setInterval(function() {
					var image = firstImage.get(0);
					if (image.complete || image.readyState == 'complete' || image.readyState == 4) {
						clearInterval(checkforloaded);
						jQuery('#flexslider-<?php echo esc)js( the_ID() ); ?>').flexslider({
							animation: "fade",
							slideshow: true,
							directionNav: true,
							controlNav: true,
				...
				<ul class="slides">
					<?php foreach ( $images as $image ): ?>
						<li>
							...
							<img src="<?php echo trim( $imageid[0] ); ?>" alt="<?php echo esc_attr( $image->post_title ); ?>">
							
							<?php if ( $image->post_excerpt ): ?>
								<div class="image-caption"><?php echo esc_html( $image->post_excerpt ); ?></div>
							<?php endif; ?>
						</li>
					<?php endforeach; ?>
				</ul>
...
<?php if ( has_post_format( 'link' ) ): // Link ?>
	...
	<div class="post-format">
		<div class="format-container pad">
			<p><a href="<?php echo (isset($meta['_link_url'][0])?esc_url( $meta['_link_url'][0] ):'#'); ?>">
