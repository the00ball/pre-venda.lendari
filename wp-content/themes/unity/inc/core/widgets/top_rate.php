    ...
    class WPO_Top_Rate_Widget extends WPO_Widget {
        public function __construct() {
            parent::__construct(
                ...
                __('WPO Top Rate Widget', 'unity'),
                // Widget description
                array( 'description' => __( 'The highest rated posts on your site', 'unity' ), )
            );
        ...
        public function widget( $args, $instance ) {
            ...
            $title      = apply_filters('widget_title', empty($instance['title']) ? __('Top Rated', 'unity') : $instance['title'], $instance, $this->id_base);
        ...
        public function form( $instance ) {
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('post_type')); ?>"><?php _e('Get most rated:', 'unity'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('sort')); ?>"><?php _e('Sort by:', 'unity'); ?></label>
                <select class="widefat" id="<?php echo esc_attr($this->get_field_id('sort')); ?>" name="<?php echo esc_attr($this->get_field_name('sort')); ?>">
                <option <?php selected('bayesian_rating', $instance['sort']); ?> value="bayesian_rating"><?php _e('Overall bayesian rating (score)', 'unity'); ?></option>
                <option <?php selected('rating', $instance['sort']); ?> value="rating"><?php _e('Average rating', 'unity'); ?></option>
                <option <?php selected('votes', $instance['sort']); ?> value="votes"><?php _e('Number of votes', 'unity'); ?></option>
                </select>
            </p>
            ...
            <p>
                ...
                <?php _e('Ascending', 'unity'); ?>
                </label>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('date_limit')); ?>"><?php _e('Ignore posts older than:', 'unity'); ?></label>
                <input id="<?php echo esc_attr($this->get_field_id('date_limit')); ?>" name="<?php echo esc_attr($this->get_field_name('date_limit')); ?>" type="text" value="<?php echo esc_attr( $instance['date_limit'] ); ?>" size="3" /> <?php _e('days', 'unity'); ?>
                <br />
                <small><?php _e('(0 to ignore date)', 'unity'); ?></small>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php _e('Limit:', 'unity'); ?></label>
