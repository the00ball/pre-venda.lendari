...
<div id="comments" class="comments">
    <header class="header-title">
        <h5 class="comments-title"><?php comments_number( __('0 Comment', 'unity'), __('1 Comment', 'unity'), __('% Comments', 'unity') ); ?></h5>
    </header><!-- /header -->
    ...
    <?php if ( have_comments() ) { ?>
        <div class="wpo-commentlists">
    	    ...
    	    <footer class="navigation comment-navigation" role="navigation">
    	        <div class="previous"><?php previous_comments_link( __( '&larr; Older Comments', 'unity' ) ); ?></div>
    	        <div class="next right"><?php next_comments_link( __( 'Newer Comments &rarr;', 'unity' ) ); ?></div>
    	    </footer><!-- .comment-navigation -->
    	    <?php endif; // Check for comment navigation ?>

    	    <?php if ( ! comments_open() && get_comments_number() ) : ?>
    	        <p class="no-comments"><?php _e( 'Comments are closed.' , 'unity' ); ?></p>
    	    <?php endif; ?>
        </div>
    <?php } ?> 
	...
	<?php
        ...
        $comment_args = array(
                        'title_reply'=> __('<h4 class="title">'.__('Leave a Comment','unity').'</h4>','unity'),
						...
						'comment_notes_before' => '<div class="form-group h-info">'.__('Your email address will not be published.','unity').'</div>',
						'comment_notes_after' => '',
                        );
    ?>
	<?php global $post; ?>
